...
class ConvLSTMCell(nn.Module):
    ...
    def forward(self, input_tensor, cur_state):
        ...
        combined_conv_bn = self.bn(combined_conv)
...
class ConvLSTMFC(nn.Module):


    def __init__(self,num_classes,input_dim, hidden_dim, kernel_size, num_layers,
                 batch_first=False, bias=True, return_all_layers=False):
        super(ConvLSTMFC, self).__init__()
        self.input_dim = input_dim
        self.hidden_dim = hidden_dim
        self.kernel_size = kernel_size
        self.num_layers = num_layers
    ...
    def forward(self, input_tensor, hidden_state=None):
        # seq_len = input_tensor.size(1)
        layer_output_list, _ = self.convlstm(input_tensor, hidden_state)
        matrix = layer_output_list[0][4,:,:,:,:]
