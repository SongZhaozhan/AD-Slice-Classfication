...
from network.convgru import Res18ConvGRU
...
def train(epoch):
    ...
    for batch_index, samples in enumerate(train_dataloader):
        ...
        for i in range(len(outputs)):
            output = outputs[i][:,0]
            label = labels[i].cuda().float()
        # labels = labels.float()
            y = torch.sigmoid(output)
        # y = y.cpu().data.numpy()
            loss_part = loss_function(y, label)
            loss += loss_part
        loss.backward()
        optimizer.step()
...
if __name__ == '__main__':
    ...
    parser.add_argument('-net', type=str, default= 'resnet18_convgru-nobg', help='net type')
    parser.add_argument('-gpus', type=list, default=[0,1,2,3,4,5,6,7], help='use gpu ')
    parser.add_argument('-batch_size', type=int, default=96, help='batch size for dataloader')
    parser.add_argument('-warm', type=int, default=1, help='warm up training phase')
    parser.add_argument('-lr', type=float, default=0.01, help='initial learning rate')
    parser.add_argument('-resume', action='store_true', default=False, help='resume training')
    parser.add_argument('-data_root', type=str, default='./data/2D_seq5_s5_nobg/', help='data path root')
    parser.add_argument('-epoch', type=int, default= 250, help='the epoch num')
